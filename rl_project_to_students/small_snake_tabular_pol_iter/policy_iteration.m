...
while 1
    ...
    while 1
        ...
        for state_idx = 1 : nbr_states
            ...
            Delta = max(Delta, abs(values(state_idx) - old_value));
        end
    ...
    policy_stable = true; 
    for state_idx = 1 : nbr_states
        old_a = policy(state_idx);
        Q_values = zeros(3,1);
        for a = 1 : nbr_actions
            s_prime = next_state_idxs(state_idx, a);
            if s_prime == 0
                Q_values(a) = rewards.death;
            elseif s_prime == -1
                Q_values(a) = rewards.apple;
            else
                Q_values(a) = rewards.default + gamm*values(s_prime);
            end
        end
        [~, policy(state_idx)] = max(Q_values);

        if policy(state_idx) ~= old_a
            policy_stable = false;
        end
    end
