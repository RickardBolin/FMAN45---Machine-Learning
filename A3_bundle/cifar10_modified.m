function cifar10_starter()
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,3,8)/sqrt(5*5*8/2), 'biases', zeros(8,1)),...
        'padding', [0 0]);
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,8,16)/sqrt(5*5*8/2), 'biases', zeros(16,1)),...
        'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,32,64)/sqrt(5*5*32/2), 'biases', zeros(64,1)),...
        'padding', [0 0]);
    
    net.layers{end+1} = struct('type', 'fully_connected',...
        'params', struct('weights', randn(10,2304)/sqrt(2304/2), 'biases', zeros(10,1)));
    ...
    training_opts = struct('learning_rate', 3e-3,...
        'iterations', 5000,...
        'batch_size', 32,...
        'momentum', 0.98,...
        'weight_decay', 0.005);
    ...
    save('A3_bundle/models/cifar10_test.mat', 'net');
